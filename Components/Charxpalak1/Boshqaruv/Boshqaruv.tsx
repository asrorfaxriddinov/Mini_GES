...
const Boshqaruv = () => {
  ...
  useEffect(() => {
    const WS_URL1 = "ws://54.93.213.231:9090/micro_gs_data_blok_ws";
    ...
    const socket2 = connectWebSocket(WS_URL2, (event) => {
      try {
        ...
        setIsPressed1(data.Tepaga === 1);
        setIsPressed2(data.Pastga === 1);
        setIsPressed3(data.TEL_Shkaf === 1);
        setIsPressed4(data.restart_pc === 1);
  ...
  const handleUpPressIn = async () => {
    ...
    await Promise.all([
      sendPostRequest("Tepaga", 1),
      sendMonitorPostRequest("Tepaga", 1),
      sendPutRequest(true),
    ]);
  };
  const handleUpPressOut = async () => {
    ...
    await Promise.all([
      sendPostRequest("Tepaga", 0),
      sendMonitorPostRequest("Tepaga", 0),
  ...
  const handleDownPressIn = async () => {
    ...
    sendPostRequest("Pastga", 1);
    sendPutRequest(true);
    await Promise.all([
      sendPostRequest("pastga", 1),
      sendMonitorPostRequest("pastga", 1),
      sendPutRequest(true),
    ]);
  };
  const handleDownPressOut = async () => {
    const { canControl } = await checkActiveUsersAndEmail();
    if (!canControl) return;
    sendPostRequest("Pastga", 0);
    sendPutRequest(false);
    await Promise.all([
      sendPostRequest("pastga", 0),
      sendMonitorPostRequest("pastga", 0),
      sendPutRequest(false),
    ]);
  };
  const toggleSwitch = () => {
    ...
    sendPostRequest("projektor_on_off", newState ? 1 : 0);
  };
  const handleRestartPress = () => {
    const newState = !isPressed4;
    sendPostRequest("restart_pc", newState ? 1 : 0, true);
  };
  const handleRejimPress = async () => {
    const { canControl } = await checkActiveUsersAndEmail();
    if (!canControl) return;
    sendPostRequest("TEL_Shkaf", !isPressed3 ? 1 : 0);
  };
